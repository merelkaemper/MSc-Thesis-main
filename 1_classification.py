...
def get_edge_slice(data, f_train_e=0.7, seed=30):
    """ xxx """
    df = data
    # ADDED: changed this to a list
    edges = list(set(df.Edge.unique()))
    random.seed(seed)
...
# Added features set for Node Centrality Measures    
def NCM(train, test):
    name = 'Air_Classification_NCM'
    features = ['Degree Centrality Source', 'Degree Centrality Target', 'Closeness Centrality Source', 
                'Closeness Centrality Target', 'Node Strength Source', 'Node Strength Target']
    simultaneous_test(train, test, features, best_params, name = name)
    nonsimultaneous_test(train, test, features, best_params, name = name)
    all_shap_values(train, test, features, best_params, name = name)
...
if __name__ == "__main__":
    ...
    # ADDED: changed this path to only US 
    for data_path in ['/Users/merelkamper/Documents/MSc Data Science/Thesis/transportation_network_evolution-master/data/features/usair_2004_2021.csv']:
