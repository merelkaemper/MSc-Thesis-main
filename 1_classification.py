...
sys.append('/Users/merelkamper/Documents/MSc Data Science/Thesis/MSc-Thesis-main/adapted/src')
from plot_style import *


# Function to split the dataset into train and test based on edge slices
...
# Function to convert DataFrame to X and Y arrays
...
# Function for simultaneous testing of classification models
...
# Function for non-simultaneous testing of classification models
...
# # Function to calculate SHAP values for feature importance
...
# The Four different feature vectors used for the classifier 
# 1. Function to test classification models with basic topological features (BTF)
def BTF(train, test):
    name = 'Train_Classification_BTF'
...
# # 2. Function to test classification models with weighted topological features (WTF)
# def WTF(train, test):
#     name = 'Train_Classification_WTF'
#     features = []
...
# 3. Function to test classification models with the edge weights (WWW)
def WWW(train, test):
    name = 'Train_Classification_WWW'
    features = ['Curr FWeight']
    simultaneous_test(train, test, features, best_params, name = name)
    nonsimultaneous_test(train, test, features, best_params, name = name)
    # all_shap_values(train, test, features, best_params, name = name)

# # 4. Function to test classification models with unweighted topological features and edge weights (BTFW)
# def BTFW(train, test):
#     name = 'Train_Classification_BTFW'
#     features = ['Common Neighbor', 'Salton', 'Jaccard', 'Sorensen', 'Hub Promoted',
#                'Hub Depressed', 'Leicht Holme Newman', 'Preferential Attachment',
